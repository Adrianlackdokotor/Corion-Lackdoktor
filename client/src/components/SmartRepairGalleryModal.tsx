...
import { smartRepairPhotos, type GalleryPhoto } from "@/data/smartRepairGallery";

interface SmartRepairGalleryModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialIndex?: number;
}

export function SmartRepairGalleryModal({ open, onOpenChange, initialIndex = 0 }: SmartRepairGalleryModalProps) {
  const [currentIndex, setCurrentIndex] = useState(initialIndex);

  const currentPhoto = smartRepairPhotos[currentIndex];

  const goToPrevious = () => {
    setCurrentIndex((prev) => (prev === 0 ? smartRepairPhotos.length - 1 : prev - 1));
  };

  const goToNext = () => {
    setCurrentIndex((prev) => (prev === smartRepairPhotos.length - 1 ? 0 : prev + 1));
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent 
        ...
        data-testid="modal-smart-repair-gallery"
      >
        <DialogHeader className="sr-only">
          <DialogTitle>Smart Repair Galerie</DialogTitle>
          <DialogDescription>
            Durchsuchen Sie unsere Sammlung von {smartRepairPhotos.length} Smart Repair Projekten
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="border-t bg-background p-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground" data-testid="text-photo-counter">
                Foto {currentIndex + 1} von {smartRepairPhotos.length}
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {smartRepairPhotos.map((photo, index) => (
              <button
                key={index}
